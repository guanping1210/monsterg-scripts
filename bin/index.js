...
function runEject() {
  ...
  ['build.js', 'utils.js'].forEach(filename => {
    if (!_fsExtra.default.existsSync('test')) {
      _fsExtra.default.ensureDir('test');
    }

    (0, _ncp.default)((0, _config.resolveScriptPath)(`scripts/${filename}`), `test/${filename}`, function (err) {
      if (err) {
        return console.error(err);
      }
    });
